...
## 2026-06-15 — datalake-redesign DL-5c-2: inspect/MCP 프로덕션 seam 배선 (CC 초안 — Master 확정 대기)

> **D-206 (CC 초안 — Master Step 0 보고 검토 후 본문 확정·교체 예정)** | DL-5c-2 inspect/MCP seam 배선방식 = **main.py 단일 PG 권위(A안)**. main.py 두 호출부가 `resolve_dataset_path(id)`로 data_path 1회 취득 → `inspect(...,data_path)` + `execute(...,data_path)` 공통 전달. inspector·MCP(server.py/tools.py)는 optional `data_path` 받기만(PG 비의존, `_resolve(dataset_id, data_path)` early-return). MCP 7종 계약 시그니처 무파괴(data_path optional query param·additive). csv 3종(timeseries/order/event-log) 적용. **image=5c-3 분리(B 결과)** — resolver(`*.csv` glob)·executor image 경로가 csv와 구조 상이(디렉터리/이미지)+5c-1 미배선이라 end-to-end 흡수 부적합(MCP server/tools 패턴은 공통이라 추후 흡수 가능). inspector(agents/inspector)는 5a 동결 대상이었으나 seam additive 예외 전환(test_inspector_seam_additive_only 신설, schemas.py·프로파일/flags 로직 동결 유지). 게이트(C 결과)=execute_endpoint 실태우기 + `_mcp_get` in-process MCP tools 우회(MCP HTTP 실기동=live smoke=명선 영역). | 근거: PG가 데이터 위치 단일 권위(5c-1 D-204 연장) — inspector/MCP는 위치를 받기만 해 PG 비의존 유지·계약 보존. |
