...
| Stage | UI 박스 단위 — 한 박스 = 1 Node 고정 (Line별 max_stages 정의) |  |
| Node | 공정 노드 — Stage 와 1:1 대응. 18종 (Line 1=6 + Line 2=5 + Line 3=7) |  |
| Module | Stage 박스 안의 Function 차원 데이터 슬롯 — 1~3개. 빈 슬롯은 없는 것 처리 |  |
| Dataset | 실제 raw 파일 |  |
...
| STEP 1A (1층 오류 수정 — modality_guess, width/height 중복 등) | 진행 중 (blueprint Part 11-1 로드맵) |
| STEP 1B (Harness 검증 강화 + Module Catalog + 6 페이지 UI) | 미진입 (알파 진입 후) |
| 알파 단계 | 미진입 (CHANGELOG 비어 있음) |
| 베타 단계 | 미진입 |
| 운영 단계 | 미진입 |
