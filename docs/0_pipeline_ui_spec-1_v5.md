...
```python
{
  ...
  # ★각 에이전트 판단 기록 (사용자 강조 — Page 4 의 표준화 과정 전체)★
  "agent_records": [
    {
      "stage_order": int,
      "module_function": str,
...
카드만으로 의사 표현 부족 시 사용. LLM 자유도 ★출구만 제한★ 으로 환각 방어.
...
| 영역 | 환각 방어 |
|---|---|
| ① | stage_context 는 ★요약된 사실★ 만 누적 (raw 데이터 안 들어감). Validator 가 누락된 lineage 검출 |
| ② | constraints = 사용자 입력 = 진실. LLM은 ★비교★ 만 ("위배 여부") |
| ③ | 질문 후보는 ★Module Catalog 기반 결정론★. LLM은 ★1~2개 추천★ + 사용자 최종 선택 |
| ④ | EDA 수치는 ★결정론 계산★. LLM은 자연어 ★표현★ 만 |
...
### 1-9-1. KAMP 등록 시나리오 (팀원 선택 결정 필요)

명세서는 두 시나리오 모두 명시. 팀원이 작업 환경 보고 선택.

#### 시나리오 A: 데이터 이전 (`data/synthetic/` → `data/lake/kamp/`)

| 장점 | 단점 |
|---|---|
| 일관된 디렉터리 구조 (모든 데이터셋 동일 패턴) | 기존 MCP `tools.py`의 `DATA_ROOT` 환경변수 ★모두 수정 필요★ |
| 신규 등록과 KAMP가 대등하게 보임 | `docker-compose.yml` 볼륨 마운트 경로 ★모두 수정★ |
| `metadata.json` 표준화 → 자동 카탈로그 깔끔 | 팀원 기존 코드 동작 깨질 위험 (작업 중이면 충돌) |
| 사용자 신규 등록 시 같은 코드 경로 재사용 | 마이그레이션 작업 자체 비용 (~2~3시간) |

#### 시나리오 B: 카탈로그 메타만 추가 (`data/synthetic/` 그대로)

```
data/synthetic/           ← 기존 그대로
+ datalake.entries        ← DB 테이블만 신규
  data_path 가 기존 synthetic/... 가리킴
```

| 장점 | 단점 |
|---|---|
| 기존 코드 변경 0 — 팀원 작업과 충돌 없음 | 디렉터리 구조 혼재 (synthetic / registered 분리) |
| 마이그레이션 비용 0 — 명세 갱신만 | KAMP `metadata.json` 자동 생성 불가 (DB에만 메타) |
| 점진 도입 — 신규 등록만 `data/lake/registered/` 사용 | 영업 시 "두 가지 데이터 경로 설명" 필요 |
| 위험 최소 | 장기적으로 시나리오 A로 이전 권장 |

#### 결정 권유
팀원에게 한 줄 질문: *"기존 `DATA_ROOT` 환경변수 수정해도 괜찮으세요? (작업 중이면 B 권장)"*
...
본 명세서를 따라 구현 시 발생 가능한 오류를 ★사전 인지★ 하기 위한 체크리스트:

#### 구현 ★사전★ 확인 (작업 시작 전)
- [ ] 1-9-1 KAMP 등록 시나리오 A/B 결정
...
- 목표: 실용성 + 가치 증명
- 검증 항목:
  - [ ] Part 1-9-1 KAMP 등록 시나리오 (운영 환경 시나리오 A/B 최종 결정)
...
★Phase 3 의 8 검증 항목은 모두 ★알파 단계★ 에서 발견·갱신★. 명세서 v4 는 ★알파 진입 시점의 잠정안★, 알파 통과 후 v5 갱신.
...
- `data/lake/{kamp,registered}/` 통합 인터페이스
- Mode A (업로드, 100MB 이하) / Mode B (서버 경로, 대용량) — 사용자 입장 단일 동작
- 시나리오 A (이전) vs B (메타만, 권장) — 팀원 결정 (Part 1-9-1)
