...
| **공정 노드 (Node)** | `Node` | 사용자 "모듈" / 기존 "공정" / blueprint v1 "Module" | Line 안 세부 (사출성형, CNC 등) |
| **단계 (Stage)** | `Stage` | (신규) | UI 박스 단위 (한 박스에 1+ Node) |
| **모듈 (Module)** | `Module` | (재정의) | Node 안의 Function 차원 데이터 슬롯 (process/quality/maintenance/reference) |
...
| 2 | `catalogs/modules.yaml` 작성 | 10+ Node 도메인 지식 (constraint_keys 구조 — v4 결정: typical_ranges 디폴트 없음) | `repo/catalogs/` |
...
| Stage | UI 박스 단위 — Line 안 1+ Node가 들어가는 단위 |
| Node | 공정 노드 — Stage 안의 세부 공정 (사출성형, CNC 등) |
| Module | Function 차원 데이터 슬롯 — Node 안 1~3개 (process/quality/maintenance/reference) |
| Dataset | 실제 raw 파일 |
...
**작성**: 2026-05-27 v5 (Claude Opus 4.7, byeonggab89 본진 코드 직접 확인 + 사용자 6단계 비전 + 옵션 E 합의 + 7가지 피드백 반영 + 산출 6 파일 v5 통일)
