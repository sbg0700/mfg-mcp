...
def ollama_unload(model: str) -> None:
    """모델 언로드 (keep_alive=0) — 다음 측정의 baseline 깨끗하게."""
    try:
...
def main():
    print("=" * 70)
    print("측정 시작 — 6 조합 (e4b/26b × 184/43k/210k)")
    print("=" * 70)
    rows = []
    for model in MODELS:
        for ds in DATASETS:
            ollama_unload(model)               # baseline 정리
            rows.append(measure_one(ds, model))
        ollama_unload(model)                   # 모델 교체 전 언로드

    # CSV 출력
    out_csv = "/tmp/measure_results.csv"
