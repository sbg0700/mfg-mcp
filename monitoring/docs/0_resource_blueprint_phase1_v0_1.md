# LLM 리소스 모니터링·프로파일링 — 마스터 블루프린트 · phase-1 (MCP) (v0_1)

> **범위 (★)**: 본 문서는 **phase-1 — 앞단 MCP·Agent 파이프라인 LLM 측정** 전용이다.
> **phase-2(EDA·모델링·코드생성 LLM) 측정은 phase-1 완료 후 STEP 3에서 main 구현 코드를 확인해 별도 문서로
> 다룬다**(여기선 다루지 않음).
>
> **문서 목적**: 이 브랜치(`feature/llm-profiling`)의 phase-1이 **무슨 프로젝트이고, 무엇을 어떤 순서로
...
> 통합**(MASTER_CONTEXT 승격 + blueprint 서사 흡수) 후 **phase-1로 범위 한정**. 이전 문서는 retire.
>
...
### 한 줄 정체성
한계 하드웨어(RTX 3070 **8GB VRAM**)에서 **목표 모델 26b가 현 하드웨어로 활용 가능한지를 데이터로
판정**하고, 가능하면 **조절 가능한 파라미터·하네스로 어디까지 효율을 끌어올릴 수 있나**, 불가하면
**같은 8GB의 현실 대안(e4b·e2b)과 필요 하드웨어**를, **앞단 MCP·Agent 파이프라인 LLM(phase-1 워크로드)**의
gemma4 **자원·성능 실측**으로 답하는, 본체에서 분기한 독립 작업 브랜치. (EDA·모델링 LLM은 phase-2.)
...
> 같은 공유 서버의 무관한 다른 프로젝트와 이름·성격이 섞이면 안 된다 — 이건 스트림 처리가 아니다.
...
- 본 브랜치: `feature/llm-profiling` 체크아웃·푸시됨. **phase-1 측정 설계 단계**(측정 코드 미작성, 측정 미시작). **phase-2(EDA·모델링)는 STEP 3 후 별도 문서.**
- 측정 환경 `[실측]`: `/api/ps`·`/api/generate`·`nvidia-smi` userspace 접근 동작 확인. 모델 메타 확인.
- 미설치: `gemma4:e2b`(8GB GPU 100% 적재 유일 후보) → **공유 Ollama 데몬에 pull(모델스토어 쓰기라 admin/byeonggab89 조율; pull 경로 CLI 불가[실측]/HTTP `/api/pull` 가능성 `[추론: HO1 확인]`)**. pull 1회로 공유 — 내 홈 별도 적재 불필요.

### 차별점 (정직 버전)
- **공식·재현 가능·방어 가능한 데이터**만 쓴다(비공식 fine-tune 배제 → Part 1-5).
- **모델 비교가 아니라 동일조건 최적화 전후**로 N%를 주장한다(→ Part 5).
- **26b(목표 모델)가 8GB로 안 됨을 데이터로 확정**하되, 이를 *결말이 아니라 출발점*으로 삼아 —
  파라미터·하네스로 8GB 실용권 진입이 가능한지, 안 되면 어떤 하드웨어/대안 모델인지까지 답한다(→ Part 6-3).
- **negative result도 정당한 산출**로 인정한다.
...
> **핵심: 측정이 막힌 게 아니다.** 필요한 측정은 전부 userspace로 가능하다. 아래 표의 일부 경로(CLI·
> journal·docker)가 admin/byeonggab89 영역이지만, 각 항목은 **측정 영향 없음 / 우회 가능 / 팀원 협조**로
> 해소된다. 즉 이 절은 "측정 가용성 제약"이 아니라 **"측정 경로를 userspace HTTP API로 고정"하는 설계 결정**이다.

| 경로 | 측정 주체 접근 `[실측]` | **측정 영향 / 해소** |
|---|---|---|
| `curl localhost:11434` (Ollama HTTP API) | 가능 | **핵심 경로** — in-band 메타·`/api/ps`·controlled 직접 호출 |
| `nvidia-smi` (per-process VRAM 포함) | 가능 | **핵심 경로** — 자원 샘플링·헤드룸 계산 |
| `pynvml` / `psutil` (userspace) | 가능(추정) `[추론]` | in-process 샘플러 (HO1에서 import 검증) |
| `ollama` CLI | admin 소유(snap) | **영향 없음** — 필요 기능 전부 HTTP API로 대체(실측도 API로 수행) |
| `journalctl -u ollama` | 그룹 권한 밖 | **영향 없음** — 버퍼 분해는 부가 진단일 뿐, `/api/ps size_vram`+nvidia-smi로 충분 |
| `docker ps/exec/inspect` | admin/byeonggab89 소유 | **passive 시계 검증에만 필요** — controlled(N% 출처)는 docker 무관. 필요 시 **byeonggab89 협조로 해소** |

> **설계 귀결**: 프로파일러는 **HTTP API + nvidia-smi/pynvml/psutil로만** 동작한다. 이 경로만으로 모든
> KPI가 측정되며(실측 검증됨), CLI·journal·docker는 측정의 *대체 가능한 부가 경로*다. 이 고정이 오히려
> 측정을 단순·견고하게 만든다(Part 4-3 controlled 직접 호출).
...
### 3-4. 알파 성능 `[byeonggab89, 알파]` (개발 중 참고치 — baseline 아님, 부록 B 참조)
| 작업 | e4b | 26b |
|---|---|---|
...
- 측정 환경: 우리가 쓸 포트 외 타 리스닝 없음(충돌 없음). 측정 전 점검은 §7(타 모델·프로세스 미점유) 참조.
...
## Part 5. 모델 서사 + baseline + "N% 개선" 프레이밍 (정직성의 심장)

### 5-0. 핵심 서사 (★ — 이 브랜치가 답하는 질문)
> **목표 모델은 26b다.** 그런데 현 8GB에선 활용 불가임을 **데이터로 증명**한다(negative result, Part 6-3).
> 그 위에서 두 질문에 실측으로 답한다:
> 1. **같은 8GB에서 조절 가능한 파라미터·하네스 제약(num_gpu·num_ctx·keep_alive·양자화)으로 26b를
>    실용 범위로 끌어올릴 수 있나?** — 가능하면 그게 결론(강한 긍정), 불가하면 "8GB로는 파라미터·하네스로도
>    26b 불가"가 결론(정직한 부정). **데이터가 결정한다.**
> 2. **그렇다면 현 8GB의 현실 대안은?** — e4b(균형)·e2b(GPU 100% 적재 하한)를 동일 조건으로 측정해
>    "이 하드웨어로 할 수 있는 것"의 범위를 긋고, **26b를 가려면 필요한 하드웨어**(24GB+)를 명시한다.
>
> 즉 26b는 *앵커가 아니라 주인공*이고, negative result는 *결말이 아니라 출발점*이다.

### 5-1. baseline 정의 (사용자 합의)
> **baseline = "각 모델이 측정 환경 구축 후, protocol 통제 조건에서 무튜닝(레버 미적용)으로 도는
> controlled 수치".** 1줄 근거: *8GB VRAM 초과 시 Ollama가 자동 CPU offload — 현 하드웨어의 실제 조건.*
> **확정 baseline은 HO3에서 재현성(CV) 확인된 controlled 수치만 인정**(Part 3-4·부록 B의 개발 중 값은
> baseline이 아니라 sanity 참고치 — 부록 B 주의 참조).

### 5-2. 모델 역할 (★ — 26b 주인공 기준 재정의)
| 모델 | 역할 | 최적화·N% 대상? |
|---|---|---|
| **26b** | **목표 모델(주인공)** — 8GB 가용성 판정 + 파라미터·하네스로 실용권 진입 가능성 시험 | **예 — 단 §9-4 강등 규칙 내에서**(스래싱 시 negative-result로) |
| e4b | 8GB **균형 현실선** — 동일모델 내 최적화 여지 가장 큼 | 예 (동일모델 전후, 분해 N% 주력) |
| e2b | 8GB **하한**(GPU 100% 적재 유일) — "현 하드웨어로 가능한 최소" | 보조(keep_alive) + **품질 게이트 통과 시에만 대안 자격**(HO1 스모크) |
...
- **통제 config ≠ 운영값 `[실측 전제]`**: backend가 옵션을 안 set해 운영은 Ollama 기본(temp~0.8 등)으로 돈다.
  baseline은 **재현용 통제 config(temp=0 등)에서 레버 미적용**이지 literal 운영값이 아니다. 운영 그대로의
  분포는 passive(A)가 별도 측정해 **섞지 않고 함께 보고**(회의론 방어 — 정직성 핵심, protocol §10).

### 5-4. 산출되는 영업·포트폴리오 자산
- **목표 vs 현실 매트릭스**: 26b(목표) — 8GB 가용성 판정 결과 + 파라미터·하네스 최적화 후 도달점 / 8GB
  현실 대안 e4b·e2b(품질 게이트) / 26b 가려면 필요 하드웨어(24GB+).
- **"같은 8GB에서 조절 가능한 제약(파라미터·하네스)으로 효율을 N% 개선"**을 동일모델 전후로 입증(주력 e4b,
  가능하면 26b도).
...
- **measure: num_ctx baseline은 4096이 아니라 실제 적재 기본값** `[실측: backend/llm.py가 num_ctx 미설정 →
  Ollama 기본값이 baseline]`. protocol에서 **적재 기본값 `[OPEN: HO1 확인]` vs 2048**로 측정. **만약 기본이
  이미 ≤2048이면 이 실험은 무의미 → skip**(H1 demote 이중 확정). 본문엔 "효과 작을 것 + 이유" 기술(해석용).
  **[OPEN: HO1 적재값 확인 후 본문 확정 — v0_2]**

### 6-3. 26b (목표 모델) — MoE 스래싱 판정 + 8GB 실용권 진입 시험 `[실측 메타 + 알파]`
26b는 MoE(128 experts, 토큰당 8 활성). 8GB에선 expert 가중치 대부분이 CPU/RAM에 있고 **토큰마다 필요한
8개가 바뀌어 CPU↔연산 랜덤 페치 폭발 → 120초 타임아웃**(알파). 이것이 목표 모델의 현 하드웨어 현실이다.
- **시험할 것 (출발점)**: 같은 8GB에서 **파라미터·하네스(num_gpu·num_ctx·keep_alive·양자화 Q3/Q2)로
  타임아웃을 실용 범위로 끌어내릴 수 있나.** `[추론]` MoE 스래싱은 expert 페치가 병목이라 파라미터로
  잡기 어려울 가능성이 크지만 — **데이터로 판정**한다(가정 금지).
- **두 갈래 결론 (데이터가 결정)**: ① 하네스로 실용권 진입 = 강한 긍정 결론 / ② 못 함 = "8GB로는
  파라미터·하네스로도 26b 불가, 24GB+ 필요" = 정직한 부정 결론. **둘 다 valuable.**
> **26b 측정 예산 (조건부 강등, §9-4)**: 26b를 *시험*하되, 스래싱이 측정을 마비시키면 **단일 호출이
> T=60s 초과 또는 써멀 스로틀 시 → negative-result 확인용(단일 프롬프트 N=3 + 타임아웃·분배 문서화)으로
> 자동 강등.** (T 근거·규칙 = protocol §9-4: e4b<10s / 26b 120s → 60s가 e4b 오탐 없이 26b만 잡음.)
> 풀 배터리 강행 금지(공유 박스 수 시간 스래싱 방지). **단 양자화(Q3/Q2)로 적재율을 바꾸는 시험은
> 강등과 별개로 시도 가능**(시간 허용 시) — 26b를 8GB에 넣는 유일한 구조적 레버일 수 있어서.
...
- Grafana 등 실시간 대시보드는 **불필요**(측정은 캠페인 후 정적 분석 — 실시간 모니터링 대상 아님). **v0_1 제외.**
...
| 권한 제약(userspace) | CLI·journal·docker는 admin/팀원 영역 | **측정엔 영향 없음** — 전부 HTTP API+nvidia-smi/pynvml/psutil로 측정, docker만 passive 시계검증에 팀원 협조 (Part 1-4) |
...
- [ ] **e2b pull 시점**: 공유 데몬에 pull(admin/byeonggab89 조율), 적재확인은 단독창.
...
- **"리소스 모니터링" ≠ "스트림 파이프라인"** — Kafka/Flink 연상 금지. 서버의 무관한 다른 프로젝트와 혼동 금지.
...
## 부록 B. 개발 중 참고치 (sanity check — **baseline 아님**) `[byeonggab89, 알파 개발 중]`
> ⚠️ **이 값들은 baseline이 아니다.** 측정 환경·도구 정리 전, protocol 통제 조건(temp·num_ctx·N·단독창)
> 적용 전에 개발 중 잰 수치다. **확정 baseline은 HO3에서 (1) 실행파일·코드 정리 완료 후 (2) protocol 조건으로
> controlled 측정 (3) CV로 재현성 확인된 수치만 인정**한다(Part 5-1). 아래는 자릿수·방향 sanity check + 진단
> 가설(26b 스래싱 등)의 근거로만 쓴다. **`[측정 후 v0_2]`에서 확정값으로 대체.**
...
| /model recommend (LLM 1회) | 9.8s | — |
| Page 4 파이프라인 (LLM ~8회) | 46.6s | 274.2s |
| ollama ps 적재 `[실측]` | ~9.87GB (GPU 3.20 / CPU 6.67, 32%GPU) | ~20GB (63%CPU/37%GPU) |
| 하드웨어 `[실측]` | RTX 3070 8GB / RAM 32GB(가용~27.6GB) / Swap 8GB / CPU 16코어 | |

## 부록 C. 측정 protocol v0_1 — 9블록 고정값 (요약 + 포인터)
> 상세·서명은 별도 `0_resource_protocol_phase1_v0_1.md`. 아래는 본 블루프린트가 고정한 값 요약.

| 블록 | 확정값 |
|---|---|
| 1 양자화 | **Q4 + e2b 기본**, Q3/Q2 선택(시간 남으면) |
| 2 프롬프트 | 실 3종(analyze/model/planner) byeonggab89 캡처·고정 + 동일 AggregatedContext + **입력 데이터셋 실재화 `[OPEN: HO1]`**(예시 press_forming은 미실재·합성CSV는 gitignore → 재생성 seed+sha256 또는 fixtures 커밋, protocol §2-2) |
| 3 제어값 | temp=0, seed=42, num_predict=512, **num_ctx=적재 기본값[OPEN] vs 2048**(backend 미설정 → 기본이 baseline), N=7, 워밍업1 콜드분리, **cache cold/warm 둘 다**, 중앙값±CV(>15% 경고) |
...
| 5 baseline | **HO3 재현성(CV) 확인된 controlled 수치만 확정**(부록 B는 baseline 아닌 참고치) + 역할(Part 5-2) + 근거 1줄(Part 5-1) |
...
**다음 단계**: (1) ~~protocol 작성~~ 완료(`0_resource_protocol_phase1_v0_1`) (2) **핸드오프 4분할 `1_HO1~4_phase1_v0_1` 작성** (3) protocol §11 OPEN(프롬프트·핀 커밋·데이터셋·num_ctx 적재값) 채움 → §12 서명 (4) 단독창 조율 → 1_HO1 진단 착수.
