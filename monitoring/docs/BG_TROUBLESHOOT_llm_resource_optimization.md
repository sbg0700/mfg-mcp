...
- **call_id로 조인**: 본진 LLM 호출에 `call_id`(uuid)와 `origin_ts_ns`(int64, `time.clock_gettime_ns(time.CLOCK_MONOTONIC)` — C3 교정, perf_counter 아님)를
...
> ⚠️ **권한 제약 (blueprint Part 1-4 우선)**: 측정 주체는 **userspace only** — `ollama` CLI(`ollama ps`/`ollama pull`)·`docker exec`·`journalctl`(아래 본문 예시) **불가**. 모두 **`/api/ps`·`/api/generate`(HTTP) + `nvidia-smi`/`pynvml`/`psutil`** 로 대체하고, `gemma4:e2b` pull은 admin/byeonggab89 협조로 한다. (프로세스 간 시계 조인은 `CLOCK_MONOTONIC` — C3 교정, blueprint Part 4-4.)
...
> ⚠️ **갱신됨 (blueprint Part 4-3 우선)**: 이 훅은 **passive 전용**이다. **controlled(B)는 프로파일러가 `/api/generate`를 직접 호출**하므로 훅이 필요 없다(권한 제약 + 단일 프로세스 시계). 아래 훅은 실트래픽 관측(passive)에만 쓴다.
...
async def generate(prompt, system=None, fmt_json=False, model=None) -> str:
    ...
    origin_ts_ns = time.clock_gettime_ns(time.CLOCK_MONOTONIC) if PROFILE else None  # C3 교정: perf_counter는 프로세스 간 비교 불가
...
def main():
    ...
    with open(OUT, "a") as f:
        while True:
            ts = time.clock_gettime_ns(time.CLOCK_MONOTONIC)  # C3 교정: 훅과 동일 단조시계라야 조인 성립
...
### 4-2. 핵심 가설 (측정으로 검증)
> ⚠️ **갱신됨 (blueprint Part 6-1/6-2 우선)**: H1은 **demote**됐다 — e4b의 CPU 잔류(~6.67GB)는 KV가 아니라 **PLE(256) + 안 쓰는 vision/audio 타워**라 `num_ctx`를 줄여도 안 줄고, e4b는 8GB에서 **100% GPU 불가**(100%는 e2b만). **첫 진단 = `num_ctx`가 아니라 `num_gpu` 스윕(Part 6-1).** 아래 H1은 보조로 측정만 한다.
...
- 최종 집계: **parquet** (분석 편의) 또는 SQLite. 시계열 + 호출별 요약 2테이블. *(→ 최신: parquet + **DuckDB**(SQLite 대안), 본진 PG 안 씀 — blueprint §7-2)*
