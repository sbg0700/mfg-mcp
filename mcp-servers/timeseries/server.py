"""
mcp-servers/timeseries/server.py
================================
timeseries MCP 서버 — 7종 도구를 HTTP 엔드포인트로 노출 (수제 prototype).

⚠️ 이것은 '진짜 MCP 프로토콜'의 stand-in이다 (CLAUDE.md §4).
   계약(7개 도구 시그니처)이 동일하므로, 나중에 실제 MCP 프로토콜로 교체해도
   상위(Agent)는 영향받지 않는다.

실행: uvicorn server:app --host 0.0.0.0 --port 8101
"""
...
app = FastAPI(title="MCP timeseries", version="0.1.0")


@app.get("/health")
def health() -> dict[str, str]:
    return {"status": "ok", "modality": "timeseries"}
...
def ep_detect_encoding(dataset_id: str, data_path: str | None = None) -> dict[str, Any]:
    try:
        return tools.detect_encoding(tools._resolve(dataset_id, data_path))
    except FileNotFoundError as e:
        raise HTTPException(404, str(e))
